...
char *mx_request_db2(char *request, t_table *tmp, t_table *table);
char *mx_request_db(t_table *tmp, char *request);
void mx_cycle_st(char *request, t_table *tmp);
void mx_cycle_sec(char *request, t_table *tmp);
...
static void mx_create_table(t_names_db_lib lib_db, t_table *table) {
...
t_sql_resp *mx_select_table(t_names_db_lib db_lib, t_table *table, char *condition, int columns) {
    int res_open = sqlite3_open(db_lib.name_db, db_lib.db);
    t_table *tmp = table;
    char *request = NULL;
    ...
    mx_stradd(&request, db_lib.name_table);
    mx_stradd(&request, " ");
    mx_stradd(&request, condition);
    mx_stradd(&request, ";");
    
    sqlite3_open(db_lib.name_db, db_lib.db);
    sqlite3_prepare_v2(*db_lib.db, request, mx_strlen(request) + 1, &response, NULL);
...
int main (int argc, char *argv[]) {
    ...
    t_table *column = NULL;
    t_sql_resp *res = NULL;
    t_names_db_lib db_lib;
    db_lib.name_db = argv[1];
    db_lib.name_table = argv[2];
    db_lib.db = &db;
    ...
    mx_create_db(db_lib.name_db); //созадем базу
    mx_create_table(db_lib, column); // создаем таблицу
    mx_push_data_table(&column, datas); //добавляем в структуру колонок реквизит с данными по каждой колонке
    mx_insert_data(db_lib, column);  //добавляем в таблицу данные
    res = mx_select_table(db_lib, column, NULL, 2); //пятым параметром указываем условие Например "WHERE ID=1", шестой параметр - количество столбцов в таблице В данной таблице и две - id, login
